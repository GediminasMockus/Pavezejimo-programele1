...
export function RequestModal({ trip, userId, onClose, onSubmitted }: { trip: Trip; userId: string; onClose: () => void; onSubmitted: () => void }) {
  const [passengerName, setPassengerName] = useState('');
  const [phone, setPhone] = useState('');
  const [seats, setSeats] = useState(1);
  const [pickupAddr, setPickupAddr] = useState<AddressValue>({ display_name: '', lat: null, lng: null });
  const [dropoffAddr, setDropoffAddr] = useState<AddressValue>({ display_name: '', lat: null, lng: null });
  ...
  const hasDriverCoords = trip.from_lat !== null && trip.from_lng !== null && trip.to_lat !== null && trip.to_lng !== null;
  const hasRequestCoords = pickupAddr.lat !== null && pickupAddr.lng !== null && dropoffAddr.lat !== null && dropoffAddr.lng !== null;
  const detour = hasDriverCoords && hasRequestCoords ? calculateDetour(trip.from_lat!, trip.from_lng!, trip.to_lat!, trip.to_lng!, pickupAddr.lat, pickupAddr.lng, dropoffAddr.lat, dropoffAddr.lng) : null;
  ...
  const directDist = hasRequestCoords ? haversineDistance(pickupAddr.lat!, pickupAddr.lng!, dropoffAddr.lat!, dropoffAddr.lng!) : null;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (seats > (trip.available_seats ?? trip.seats)) return setFormError(`Vairuotojas siūlo tik ${trip.seats} vietas.`);
    ...
    if (trip.created_by === userId) return setFormError('Negalite siųsti užklausos į savo skelbimą.');

    const { data: existing } = await supabase.from('ride_requests').select('id,status').eq('trip_id', trip.id).eq('passenger_id', userId).in('status', ['pending', 'accepted']).limit(1);
    if (existing && existing.length > 0) return setFormError(existing[0].status === 'accepted' ? 'Šią kelionę jau pasirinkote.' : 'Šiai kelionei jau išsiuntėte užklausą.');
    ...
    setSubmitting(false);
    if (error) {
      setFormError(error.code === '23505' ? 'Šiai kelionei jau turite aktyvią užklausą.' : 'Nepavyko išsiųsti užklausos. Bandykite dar kartą.');
      return;
    }
    onSubmitted();
  }
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/60 backdrop-blur-md px-0 sm:px-4">
      <div className="w-full sm:max-w-lg bg-gradient-to-br from-white to-slate-50 rounded-t-3xl sm:rounded-3xl shadow-2xl shadow-slate-900/20 max-h-[92vh] overflow-y-auto">
        <div className="sticky top-0 bg-gradient-to-r from-emerald-500 to-teal-600 px-5 sm:px-6 pt-5 pb-4 flex items-center justify-between shadow-lg">
          <div><h2 className="text-lg font-bold text-white">Pasirinkti skelbimą</h2><p className="text-xs text-emerald-100 mt-0.5">{trip.from_location} → {trip.to_location} · {formatDateTime(trip.departure_time)}</p></div>
        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-6 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Keleivių skaičius" icon={<Users className="w-4 h-4" />}><input type="number" min={1} max={trip.available_seats ?? trip.seats} value={seats} onChange={(e) => setSeats(Math.max(1, Number(e.target.value) || 1))} className="form-input" /></Field>
            <Field label="Bagažas" icon={<Briefcase className="w-4 h-4" />}><select value={baggage} onChange={(e) => setBaggage(e.target.value)} className="form-input"><option value="">Nenurodyta</option><option value="Nėra">Nėra bagažo</option><option value="Mažas">Mažas (kuprinė)</option><option value="Vidutinis">Vidutinis (lagaminas)</option><option value="Didelis">Didelis (keli lagaminai)</option></select></Field>
          </div>
          <Field label="Pastabos vairuotojui (nebūtina)"><textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="pvz. važiuoju su vaikų kėdute" rows={2} maxLength={500} className="form-input resize-none" /></Field>
