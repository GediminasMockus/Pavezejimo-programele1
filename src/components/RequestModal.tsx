...
export function RequestModal({ trip, passengerTrip, initialRoute, userId, onClose, onSubmitted }: { trip: Trip; passengerTrip?: Trip | null; initialRoute?: CorridorSearchRoute | null; userId: string; onClose: () => void; onSubmitted: () => void }) {
  ...
  const [profileLoading, setProfileLoading] = useState(!passengerTrip);
  ...
  useEffect(() => {
    if (passengerTrip) {
      setPassengerName(passengerTrip.name);
      setPhone(passengerTrip.phone ?? '');
      setSeats(passengerTrip.seats);
      setPickupAddr({
        display_name: passengerTrip.from_location,
        lat: passengerTrip.from_lat,
        lng: passengerTrip.from_lng,
      });
      setDropoffAddr({
        display_name: passengerTrip.to_location,
        lat: passengerTrip.to_lat,
        lng: passengerTrip.to_lng,
      });
      setBaggage(passengerTrip.baggage ?? '');
