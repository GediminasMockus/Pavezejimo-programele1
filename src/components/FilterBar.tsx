  ...
  return (
    <div className="mb-4">
      <div className="flex items-center gap-2">
        ...
        {activeCount > 0 && (
          <button onClick={() => onChange(emptyFilters)} className="ml-auto min-h-11 inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700 px-2 py-1 rounded-lg">
      ...
      {open && (
        <div className="mt-3 rounded-2xl bg-white border border-slate-200 p-4 shadow-sm flex flex-col gap-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <label><span className="text-xs font-medium text-slate-500">Atstumas nuo manęs, km</span><select value={filters.radiusKm} onChange={e => onChange({ ...filters, radiusKm: Number(e.target.value) })} className="form-input mt-1"><option value={0}>Neribotas</option><option value={5}>5 km</option><option value={10}>10 km</option><option value={20}>20 km</option><option value={50}>50 km</option><option value={100}>100 km</option></select></label>
          </div>
          <label className="min-h-11 flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={filters.recurringOnly} onChange={e => onChange({ ...filters, recurringOnly: e.target.checked })} className="w-5 h-5 rounded border-slate-300 text-blue-600 focus:ring-blue-500" /><span className="text-sm text-slate-600 flex items-center gap-1"><Repeat className="w-3.5 h-3.5 text-blue-500" /> Tik pasikartojantys</span></label>
        </div>
      )}
    </div>
  );
