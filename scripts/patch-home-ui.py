...
p = Path('src/App.tsx')
s = p.read_text()
start = s.index('function HomeScreen(')
end = s.index('function ListScreen(', start)
new = r'''function HomeScreen({ userId, onPick, onSignOut }: { userId: string; onPick: (role: TripRole) => void; onSignOut: () => void }) {
